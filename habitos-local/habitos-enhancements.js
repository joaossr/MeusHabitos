(() => {
  ...
  const ensureMissions=()=>{
    ...
    importantMissions.forEach(m=>{if(!s.missions.some(x=>x.id===m.id))s.missions.push({...m});});
    write(s);
  };
  ensureMissions();
  ...
  function deleteMission(id){if(!confirm('Remover esta missão? Ela deixará de aparecer na lista.'))return;const s=read();s.missions=(s.missions||[]).filter(m=>m.id!==id);write(s);renderMissionPage();}
  function missionCards(){const s=read();return(s.missions||[]).filter(m=>m.enabled!==false).map((m,i)=>{const v=missionValue(s,m),p=Math.min(100,Math.round(v/Math.max(1,m.target)*100));return `<div class="card mh-mission ${p>=100?'complete':''}"><div class="mh-mission-head"><div><div class="eyebrow">${p>=100?'✓ Concluída':'Missão '+(i+1)}</div><h3>${esc(m.title)}</h3><p>${esc(m.description||'')}</p></div><b>+${m.reward} XP</b></div><div class="goal-row"><span>${v} / ${m.target}</span><strong>${p}%</strong></div><div class="progress"><div style="width:${p}%"></div></div><div class="mh-actions"><button class="btn mh-edit" data-mid="${m.id}">✎ Editar</button><button class="btn danger mh-delete" data-mid="${m.id}">× Remover</button></div></div>`}).join('')}
  function renderMissionPage(){const c=document.getElementById('content');if(!c)return;c.innerHTML=`<div class="mh-page"><div class="section-head" style="margin-top:0"><div><h2>Missões</h2><span>Desafios para manter seus hábitos vivos até 31/12.</span></div><button class="btn primary" id="mhNew">+ Nova missão</button></div><div class="mh-grid">${missionCards()}</div><div class="card mh-info"><b>Como usar</b><span>As missões semanais renovam o desafio automaticamente. Edite ou remova as que não fizerem sentido para sua rotina.</span></div></div>`;document.querySelectorAll('.mh-edit').forEach(b=>b.onclick=()=>editMission(b.dataset.mid));document.querySelectorAll('.mh-delete').forEach(b=>b.onclick=()=>deleteMission(b.dataset.mid));document.getElementById('mhNew').onclick=()=>editMission(null)}
  function editMission(id){const s=read(),old=(s.missions||[]).find(m=>m.id===id)||{id:'',title:'',description:'',type:'daily',target:3,reward:50,enabled:true};const title=prompt('Nome da missão:',old.title);if(title===null)return;const desc=prompt('Descrição:',old.description||'');if(desc===null)return;const types='daily | consistency | done | xp | active | perfect | habit';const type=prompt('Tipo ('+types+'):',old.type)||old.type;let habitId=old.habitId||'';if(type==='habit')habitId=prompt('ID do hábito (ex.: h1 Exercício, h2 Estudar):',habitId)||habitId;const target=Math.max(1,Number(prompt('Meta:',old.target))||old.target);const reward=Math.max(0,Number(prompt('Recompensa em XP:',old.reward))||old.reward);const enabled=prompt('Ativa? digite sim ou não:',old.enabled?'sim':'não')?.toLowerCase()!=='não';const m={...old,id:old.id||'m_'+Date.now(),title:title.trim()||old.title,description:desc,type,target,reward,enabled};if(type==='habit')m.habitId=habitId;else delete m.habitId;if(!m.title)return;const i=(s.missions||[]).findIndex(x=>x.id===m.id);if(i>=0)s.missions[i]=m;else(s.missions||=[]).push(m);write(s);renderMissionPage()}
  ...
  // O app.js usa handlers individuais. Ao filtrar a lista ele troca o innerHTML
  // e esses handlers deixam de existir. Abaixo usamos delegação para garantir
  // que Editar, Restaurar/Arquivar e Excluir continuem funcionando sempre.
  document.addEventListener('click',e=>{
    const button=e.target.closest('.item-actions [data-action]');
    if(!button || typeof window.action!=='function')return;
    e.preventDefault();
    e.stopImmediatePropagation();
    window.action(button.dataset.action,button.dataset.id,button.dataset.goal);
  },true);

  // Também reaplica os handlers do app depois de trocar o filtro/busca.
  const rebindHabitActions=()=>{if(typeof window.bind==='function')window.bind();};
  document.addEventListener('input',e=>{if(e.target?.id==='habitSearch')setTimeout(rebindHabitActions,0);});
  document.addEventListener('change',e=>{if(e.target?.id==='habitFilter')setTimeout(rebindHabitActions,0);});
