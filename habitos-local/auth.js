(() => {
  ...
  const friendly = e => ({
    ...
    'auth/operation-not-allowed':'Este método está desativado no Firebase Authentication.',
    'auth/network-request-failed':'Falha de conexão. Verifique sua internet.',
    'auth/unauthorized-domain':'Este domínio não está autorizado no Firebase Authentication.',
    'auth/invalid-api-key':'A chave de API do Firebase não corresponde a este projeto.',
    'auth/account-exists-with-different-credential':'Este e-mail já está cadastrado com outro método de login.'
  }[e.code] || e.message || 'Não foi possível concluir a operação.');
  ...
  window.addEventListener('DOMContentLoaded',async()=>{
    ...
    }catch(e){ console.error('Google redirect:',e); message(friendly(e)); }

    function setMode(next){
      mode=next; card.classList.toggle('mode-signup',next==='signup');
      signup.textContent=next==='signup'?'Voltar para entrar':'Criar minha conta';
      submit.textContent=next==='signup'?'Cadastrar':'Entrar'; message('');
    ...
    form.addEventListener('submit',async e=>{
      ...
      const email=emailEl.value.trim(),password=passwordEl.value,name=nameEl?.value.trim()||'';
      if(!email){message('Digite seu e-mail.');return;}
      if(mode==='signup'&&!name){message('Digite seu nome de usuário.');return;}
      if(password.length<6){message('A senha precisa ter pelo menos 6 caracteres.');return;}
      busy=true;submit.disabled=true;signup.disabled=true;if(google)google.disabled=true;submit.textContent=mode==='signup'?'Criando conta…':'Entrando…';
      try{
        if(mode==='login') await auth.signInWithEmailAndPassword(email,password);
          ...
          message('Conta criada com sucesso! Entrando…',true); setTimeout(goApp,250);
        }
      }catch(e){console.error('Auth:',e);message(friendly(e));busy=false;submit.disabled=false;signup.disabled=false;if(google)google.disabled=false;submit.textContent=mode==='signup'?'Cadastrar':'Entrar';}
    });

    google?.addEventListener('click',async()=>{
      if(busy)return; busy=true; message('Redirecionando para o Google…',true);
      google.disabled=true;submit.disabled=true;signup.disabled=true;
      try{
        const provider=new firebase.auth.GoogleAuthProvider();
        provider.setCustomParameters({prompt:'select_account'});
        await auth.signInWithRedirect(provider);
      }catch(e){console.error('Google:',e);message(friendly(e));busy=false;google.disabled=false;submit.disabled=false;signup.disabled=false;}
    });

    forgot?.addEventListener('click',async()=>{
      const email=emailEl.value.trim(); if(!email){message('Digite seu e-mail primeiro.');return;}
