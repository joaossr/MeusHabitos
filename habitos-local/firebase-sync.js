(function(){
  ...
  function configured(){
    ...
    return c && c.apiKey && !String(c.apiKey).startsWith("COLE_") &&
           c.projectId && !String(c.projectId).startsWith("SEU-");
  }
  ...
  async function init(){
    ...
    uid=auth.currentUser.uid; enabled=true;
  ...
  async function save(state){
    ...
    await db.collection("users").doc(uid).collection("app").doc("state").set({
      state,
