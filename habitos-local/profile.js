(() => {
  ...
  function modernizeSidebar() {
    ...
    sidebar.querySelector('[data-page="schedule"]')?.remove();
    ...
    sidebar.querySelectorAll('.nav-item[data-page]').forEach(btn => {
      ...
      const existingLabel = btn.querySelector('.nav-label');
      const label = existingLabel?.textContent?.trim() || btn.textContent.replace(/^[^A-Za-zÀ-ÿ]+/,'').trim();
